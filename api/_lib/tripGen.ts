...
//
// Deliberately NOT clamped against the client's own HARD_MAX_PLACES_PER_DAY
// (7, see generateTrip.ts) — an earlier version of this function was, on the
// reasoning that "the client-side duration walk never keeps more than 7
// places anyway, so asking for more just wastes output tokens." That
// backfires exactly on the days that need the buffer most: a packed-pace
// day's targetPlaceCount is already at 7 (its own max), so the old clamp
// pinned overAskCount right back down to 7 too — zero actual buffer. Losing
// even ONE candidate to ordinary Google Places verification attrition
// (unfindable name, wrong-city distance check, this day's own
// geographic-anchor filter in generate-trip-day.ts) then dropped the day
// below its target and triggered aiTripClient.ts's backfill round — a whole
// second Claude call plus another overAskCount worth of Google verification
// calls, just to recover the one place that was lost. The extra output
// tokens on a few more candidates (this function's worst case is 12, since
// targetPlaceCount is validated to at most 10) cost far less than one
// avoidable backfill round.
...
// The output-format rules below (categories/name/description, timeOfDay,
// estimatedTimeHours, naming conventions, geocodeQuery/geocodeQueryAlt, the
// "only recommend places you're confident are real" rule) are byte-identical
// on EVERY generate-trip-day request — nothing here depends on ctx, day, or
// pace. Split out from buildDayPrompt so generate-trip-day.ts can send this
// as a `system` block with `cache_control: { type: 'ephemeral' }` instead of
// folding it into the per-request user message: since every day-request
// across every trip shares this exact text, the first request in a cache
// window pays the ~1.25x write premium and every other one within the TTL
// (including this same trip's own other days, requested in parallel/soon
// after) reads it at ~10% of normal input-token cost instead of repaying
// full price for the same ~2000 characters of rules every single time.
// Never varies, so it takes no parameters.
...
// line, same as the old best-effort fallback. Paired with
// buildDaySystemPrompt (sent separately as a cached `system` block) — this
// function covers only what actually varies request to request, so it stays
// out of the way of that block's cache_control breakpoint (prefix-matched
// caching means anything AFTER the cached content still has to be identical
// to hit, but content before/separate from it doesn't need to be).
