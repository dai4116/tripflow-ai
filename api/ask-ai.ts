...
function buildPrompt(message: string, destination: string, columns: ColumnSummary[]): string {
  ...
  return [
    ...
    'suggest_places 每個地點另外提供 geocodeQuery 欄位：這是給地圖服務（OpenStreetMap）查詢定位用的完整字串，不會顯示給使用者。格式必須是「地點官方名稱, 城市, 國家」，三段全部使用同一種語言——當地官方語言或英文（不要中英文混用，也不要只寫地點名稱、漏掉城市和國家，否則常常查不到或定位到錯誤的地方）。只有目的地本身是華語地區時，才整段使用中文。',
    '如果不確定正式登記名稱，額外提供 geocodeQueryAlt 欄位（格式同 geocodeQuery，但用更簡短通用的說法），作為查詢失敗時的備援；有把握的話可以不用提供。',
